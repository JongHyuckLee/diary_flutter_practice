...
class Diary {
  ...
  Map<String, dynamic> toJson() {
    return {
      "text": text,
      // DateTime은 문자열로 변경해야 jsonString 으로 변환 가능하다.
  ...
  factory Diary.fromJson(Map<String, dynamic> jsonMap) {
    return Diary(
      text: jsonMap['text'],
      // 문자열로 넘어온 시간을 DateTime으로 다시 바꿔준다.
...
class DiaryService extends ChangeNotifier {
  ...
  DiaryService(this.prefs) {
    // 생성자가 호출할 때 SharedPreferences 로 저장해 둔 diaryList를 불러온다.
    // 저장할 때와 반대로 String -> Map -> Diary로 변환한다.
  ...
  List<Diary> getByDate(DateTime date) {
    return diaryList
        .where((element) => isSameDay(date, element.createdAt))
        .toList();
  }
  ...
  // 함수 이름을 _로 이름을 시작하면 DiaryService 내부에서만 호출할 수 있다.
  void _saveDiaryList() {
    // Diary라는 직접 만든 클래스는 shared preferences에 그대로 저장할 수 없다.
    // SharedPreferences에서 저장할 수 있는 String 형태로 변환을 해주어야 한다.
    // 나만의 규칙을 만들어 Diary를 String 형태로 변환할 수 있지만, 보통 json 이라는 규칙을 이용한다.
    // Diary -> Map -> String 순서로 변환한다.
